...
const int TEMP_PIN          = 35; // NTC thermistor input
...
const float TAU_MULTIPLE_POST_KNEE = 2.0f;
const unsigned long MIN_PULSE_MS = 2000, MAX_PULSE_MS = 90000;
const unsigned long MIN_REST_MS  = 3000, MAX_REST_MS  = 120000;

const float ADSORPTION_RATIO_THRESHOLD = 0.8f;   // Ratio < 0.8 marks adsorption start
const float OUTGAS_RATIO_THRESHOLD      = 0.3f;   // Ratio < 0.3 marks irreversible outgassing
...
enum PulseSubPhase {
  ...
  PULSE_REST_EARLY,
  PULSE_REST_LATE,
  PULSE_CAP_PROBE // New subphase to establish baseline at low voltage
};

enum PulseResult {
  PULSE_NONE,
  PULSE_ADSORPTION_KNEE,
  PULSE_OUTGASSING_KNEE,
  PULSE_NO_KNEE,
  PULSE_CEILING_HIT
};
...
void handleModePulseTest(float vBatt, unsigned long now) {
  ...
  // Special handling for the very first pass's capacitance establishing
  if (C_baseline < 0 && pulseSubPhase == PULSE_PRE && vBatt > 13.0f && vBatt < 13.5f) {
      Serial.println("BISE: establishing C_baseline at low voltage.");
      pulseSubPhase = PULSE_CAP_PROBE;
      pulsePhaseStartedAt = now;
      V_afterIR = 0; // Reset tracking for this subphase
  }

  switch (pulseSubPhase) {

    case PULSE_CAP_PROBE: {
       applyPulseDuty(iBatt);
       if (now - pulsePhaseStartedAt >= SLOPE_FIT_WINDOW_MS + IR_SETTLE_MS) {
           float probeSlope = (vBatt - V_afterIR) / (SLOPE_FIT_WINDOW_MS/1000.0f);
           float iAmps = iBatt/1000.0f;
           if (probeSlope > 1e-6f) {
               C_baseline = iAmps / probeSlope;
    ...
    case PULSE_MAIN: {
      ...
      if (currentChargeDuty >= PWM_MAX - 5 && vIn < (vinTarget - 1.0f)) {
        ...
        if (gasDetected) pulsePostKneeStartedAt += CONTROL_INTERVAL_MS;
        ledcWrite(CHARGE_CH, 0);
        return;
      }
      ...
      // Noise rejection: use Kalman if settled, else EMA of raw
      float usedSlope = (fabs(slope_est) > 1e-7) ? slope_est : instSlope;
      ...
      if (!gasDetected) {
        // Efficiency ratio based on established low-voltage baseline
        float ratio = (fabs(expectedSlope) > 1e-7) ? emaSlope/expectedSlope : 1.0f;

        if (vBatt >= CHARGE_V_OVERVOLTAGE - OVERVOLTAGE_PULSE_MARGIN) {
          Serial.println("BISE: ceiling hit before gassing.");
          lastPulseResult = PULSE_CEILING_HIT;
          goto pulseMainDone;
        }

        if (ratio < OUTGAS_RATIO_THRESHOLD) {
          kneeConfirmTicks++;
          if (kneeConfirmTicks >= KNEE_CONFIRM_TICKS) {
            ...
            Serial.print("BISE: GAS knee! V="); Serial.print(kneeVoltage,4);
            Serial.print(" I="); Serial.print(kneeCurrentMa,1);
            Serial.print(" eff="); Serial.println(ratio,3);
          }
        } else if (!adsDetected && ratio < ADSORPTION_RATIO_THRESHOLD) {
          kneeConfirmTicks++;
          if (kneeConfirmTicks >= KNEE_CONFIRM_TICKS) {
            adsDetected = true;
            Serial.print("BISE: ADS knee (reversible). V="); Serial.print(vBatt,4);
            Serial.print(" ratio="); Serial.println(ratio,3);
            kneeConfirmTicks = 0; // Reset for gas detection
          }
        } else {
          kneeConfirmTicks = 0;
        }
        ...
        // Adaptive extension: if we see adsorption but not gas, extend the pulse to wait for saturation
        if (adsDetected && !gasDetected) noKneeTimeout *= 1.5;

        if (!gasDetected && elapsed >= noKneeTimeout) {
          slope2 = emaSlope;
          lastPulseResult = adsDetected ? PULSE_ADSORPTION_KNEE : PULSE_NO_KNEE;
          goto pulseMainDone;
        }
      } else {
        ...
        if (postKneeElapsed >= postKneeTarget) {
          slope2 = emaSlope;
          lastPulseResult = PULSE_OUTGASSING_KNEE;
          goto pulseMainDone;
        }
      }
      break;
      ...
      if (lastPulseResult == PULSE_OUTGASSING_KNEE || lastPulseResult == PULSE_ADSORPTION_KNEE) {
    ...
    case PULSE_REST_LATE: {
      ...
      if (now - pulsePhaseStartedAt >= restRemainingMs) {
        if (pulseOpenLoopMaxDuty) {
          if (lastPulseResult == PULSE_OUTGASSING_KNEE) {
            ...
            beginBisectionPulse();
          } else if (lastPulseResult == PULSE_ADSORPTION_KNEE) {
             Serial.println("BISE: adsorption seen but no outgassing yet. Increasing pulse current.");
             beginPulse(); // Keep going with max duty
          ...
          if (lastPulseResult != PULSE_OUTGASSING_KNEE) {
            Serial.println("BISE: final confirm failed to reproduce gassing. Result may be noisy.");
          }
    ...
    Serial.print("STAT: R_est="); Serial.print(internalResistance*1000.0f, 1); Serial.print("mO C_est="); Serial.print(C_baseline, 2); Serial.println("F");
          ...
          if (lastPulseResult == PULSE_OUTGASSING_KNEE) {
            ...
            // PULSE_NO_KNEE or PULSE_ADSORPTION_KNEE (we want to search ABOVE adsorption)
...
void logTelemetry() {
  ...
  float ratio = 1.0f;
  float c_diff = 0;
  if (mode == MODE_OUTGAS_PULSE_TEST) {
    float iAmps = (ina219.getCurrent_mA() * CURRENT_SIGN) / 1000.0f;
    if (C_baseline > 0) {
      float expectedSlope = (iAmps / C_baseline);
      ratio = (fabs(expectedSlope) > 1e-6) ? emaSlope / expectedSlope : 1.0f;
    }
    c_diff = (fabs(emaSlope) > 1e-6) ? iAmps / emaSlope : 0;
  }
  Serial.print(ratio, 3); Serial.print(",");
  Serial.print(c_diff, 3); Serial.print(","); // c_diff replaces static C_baseline in logs
