...
class BatterySim:
    def __init__(self, c=100.0, r_int=0.08, outgas_v=13.9, i_parasitic=-0.050, temp=25.0):
        ...
        self.v_dl = 0.0
        self.v_gas = 0.0
        self.v_ads = 0.0  # Hydrogen adsorption (reversible)

        self.c_main = c
        self.c_dl = c * 0.05
        self.c_gas = c * 2.0
        self.c_ads = c * 0.5   # Adsorption is high capacity but limited potential range
        ...
        self.ads_v_offset = 0.15 # Adsorption starts 150mV before gassing
    ...
    def step(self, i_applied, dt):
        ...
        ads_v_now = outgas_v_now - self.ads_v_offset

        for _ in range(n_steps):
            v_base = self.v_oc + self.v_dl
            v_term_pre = v_base + self.v_ads + self.v_gas + i_applied * self.r_int

            # 1. Adsorption (Reversible pseudo-capacitance)
            # Only active between ads_v_now and outgas_v_now
            i_ads_reaction = 0
            if v_term_pre > ads_v_now and self.v_ads < self.ads_v_offset:
                # Potential-driven adsorption
                i_ads_reaction = (v_term_pre - ads_v_now - self.v_ads) * 5.0
                i_ads_reaction = constrain(i_ads_reaction, 0.0, max(0.0, i_applied))
            elif v_term_pre < ads_v_now + self.v_ads:
                # Potential-driven desorption
                i_ads_reaction = (v_term_pre - ads_v_now - self.v_ads) * 5.0
                i_ads_reaction = constrain(i_ads_reaction, -0.5, 0.0) # Desorption is slower

            # 2. Irreversible Gassing
            i_gas_reaction = 0
            # Term voltage must exceed outgas ceiling
            if v_term_pre > outgas_v_now:
                i_gas_reaction = (v_term_pre - outgas_v_now)**1.5 * self.outgas_slope
                i_gas_reaction = min(i_gas_reaction, max(0.0, i_applied - i_ads_reaction))

            i_charging = i_applied - i_ads_reaction - i_gas_reaction

            # Update States
            self.v_dl += (i_applied * self.r_int - self.v_dl) * (sub_dt / (self.r_dl * self.c_dl))
            self.v_ads += (i_ads_reaction * sub_dt) / self.c_ads
            self.v_ads = constrain(self.v_ads, 0.0, self.ads_v_offset + 0.05)

            if i_gas_reaction > 0:
                self.v_gas += (i_gas_reaction * 0.2) * (sub_dt / self.c_gas)
            else:
                # Irreversible gas effect decays very slowly (bubbles clearing)
                self.v_gas *= math.exp(-sub_dt / 500.0)

            if not self.is_stalled:
                self.v_oc += (i_charging * 0.95 * sub_dt) / self.c_main

        v_final = self.v_oc + self.v_dl + self.v_ads + self.v_gas + i_applied * self.r_int
