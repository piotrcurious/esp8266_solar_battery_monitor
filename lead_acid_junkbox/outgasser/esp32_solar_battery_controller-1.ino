...
const int PWM_DISCHARGE_PIN = 26;
const int BUTTON_PIN        = 27;
const int VIN_PIN           = 34;

// ================= LEDC (PWM) config =================
const int CHARGE_CH    = 0;
const int DISCHARGE_CH = 1;
...
const float OUTGAS_EFFICIENCY_THRESHOLD = 0.5f;  // observedSlope/expectedSlope below this = outgassing
...
void startDischargeFormation(float testCurrentMa) {
  state.setCurrent = testCurrentMa;
  pidSet_discharge = state.setCurrent;
...
void setup() {
  ...
  ledcSetup(DISCHARGE_CH, PWM_FREQ_HZ, PWM_RES_BITS);
  ledcAttachPin(PWM_DISCHARGE_PIN, DISCHARGE_CH);
  allOff();
  ...
  pidDischarge.SetOutputLimits(0, PWM_MAX);
...
void loop() {
  ...
  if (Serial.available()) {
    char c = Serial.read();
    if ((c=='D'||c=='d') && mode==MODE_IDLE) startDischargeFormation(state.outgassingCurrent_mA > 0 ? state.outgassingCurrent_mA : 100.0f);
  ...
  if (digitalRead(BUTTON_PIN) == LOW && mode == MODE_IDLE) {
    startDischargeFormation(state.outgassingCurrent_mA > 0 ? state.outgassingCurrent_mA : 100.0f);
  }
  ...
  bool activeModeUsesInput = (mode == MODE_CHARGE_BULK || mode == MODE_OUTGAS_PULSE_TEST
                               ...
                               || mode == MODE_DISCHARGE_FORMATION || mode == MODE_WAIT_AFTER_OUTGASSING
                               || mode == MODE_IDLE);
  ...
  switch (mode) {
    ...
    case MODE_DISCHARGE_FORMATION: {
      float iBatt = fabs(ina219.getCurrent_mA() * CURRENT_SIGN);
      ...
      ledcWrite(DISCHARGE_CH, (int)pidOut_discharge);

      if (fabs(slope_est) < slopeThreshold && (currentMillis - stateEnteredAt) > 5000) {
        Serial.print("Discharge plateau at voltage: "); Serial.println(voltage_est, 4);
        ledcWrite(DISCHARGE_CH, 0);
    ...
    case MODE_WAIT_AFTER_OUTGASSING:
      if (currentMillis - stateEnteredAt >= 20UL * 60UL * 1000UL) {
        state.setCurrent -= 50.0f;
        if (state.setCurrent <= 0) {
          Serial.println("Formation test complete.");
          enterMode(MODE_IDLE);
        } else {
          startDischargeFormation(state.setCurrent);
        }
      }
      break;
