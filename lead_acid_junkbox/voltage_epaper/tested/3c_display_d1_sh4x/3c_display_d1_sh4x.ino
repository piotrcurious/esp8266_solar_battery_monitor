...
#define TEMPERATURE_FONT &FreeSansBold18pt7b
#define TEMPERATURE_X 0
#define TEMPERATURE_Y 121
const float Temperature_redThreshold = 32.0;        // Display red for voltages below this

//#include "open_sans_quick_16.h"
//#define HUMIDITY_FONT &Open_Sans_Light_16

#include "Irish_Grover_Regular_32.h"
#define HUMIDITY_FONT &Irish_Grover_Regular_32

#define REL_HUMIDITY_X 125
#define REL_HUMIDITY_Y 121
const float RelHumidity_redThreshold = 65.0;        // Display red for voltages below this
...
#define REFRESH_INTERVAL 1000*60*5 // define refresh interval 
#define LOW_VOLTAGE_SLEEP 60 // define how long to sleep when voltage is low 
                              // must sleep at least for entire display refresh period (22sec)
                              // otherwise wakeup reset resets the display aswell for some reason
...
const int gpioPin = 0 ;  // GPIO10 (S3) to control
...
struct RTCData {
  ...
  uint32_t lastRefreshTime = 0 ; //last refresh time for epaper 
};
...
//    WiFi.mode(WIFI_OFF);
  ...
  Serial.println(); // after the bootloader junk
...
/*
  ...
  if (getMillis() - rtcData.lastRefreshTime > REFRESH_INTERVAL) {
//      display.refresh();  // Refresh the ePaper display to show changes
    ...
    //  rtcData.lastRefreshTime = getMillis();
      }
*/

//TODO device reset resets the display. it should be put to sleep 
      display.init(115200);
      display.powerOff();
      display.hibernate();
...
// You can have 6 different heater settings
  ...
  sht4.setHeater(SHT4X_NO_HEATER);  // setting to high heat to decontaminate upon startup
...
//  displayVoltage(batteryVoltage); // Display the current voltage on ePaper
  ...
  if (getMillis() - rtcData.lastRefreshTime > REFRESH_INTERVAL) {
      ...
      sht4.getEvent(&humidity, &temp);// populate temp and humidity objects with fresh data
       ...
       display.setTextColor(GxEPD_BLACK); // Default to black text
      display.setFont(TEMPERATURE_FONT); // Set larger font from Adafruit GFX library
      displayTemperature(temp.temperature); 
      display.setTextColor(GxEPD_BLACK); // Default to black text
      display.setFont(HUMIDITY_FONT); // Set larger font from Adafruit GFX library
      displayRelHumidity(humidity.relative_humidity); 
...
//      displayVoltage(batteryVoltage); // Update voltage on ePaper
    ...
    Serial.println("Voltage dropped below 14V, turning off GPIO4");
    pinMode(gpioPin, OUTPUT);
...
void displayTemperature(float Temperature) {
  ...
  if (Temperature > Temperature_redThreshold) {
    display.setTextColor(GxEPD_RED);  // Display red text 
 //   display.setTextColor(GxEPD_BLACK);  
  } else {
    display.setTextColor(GxEPD_BLACK);  // Display black text 
  }

  // Display the Temperature on ePaper
  display.setCursor(TEMPERATURE_X, TEMPERATURE_Y);  // Adjust position if necessary
//  display.print("Voltage: ");
  display.print(Temperature, 1);  // Print Temperature with 2 decimal places
  display.println("C");
    //display.display();  // Refresh the ePaper display to show changes
}

void displayRelHumidity(float RelHumidity) {
  // Set text color based on RelHumidity threshold
  if (RelHumidity > RelHumidity_redThreshold) {
    display.setTextColor(GxEPD_RED);  // Display red text 
 //   display.setTextColor(GxEPD_BLACK);  
  } else {
    display.setTextColor(GxEPD_BLACK);  // Display black text 
  ...
  // Display the Temperature on ePaper
  display.setCursor(REL_HUMIDITY_X, REL_HUMIDITY_Y);  // Adjust position if necessary
//  display.print("Voltage: ");
  display.print(RelHumidity, 1);  // Print Temperature with 2 decimal places
  display.println("%");
    //display.display();  // Refresh the ePaper display to show changes
}
