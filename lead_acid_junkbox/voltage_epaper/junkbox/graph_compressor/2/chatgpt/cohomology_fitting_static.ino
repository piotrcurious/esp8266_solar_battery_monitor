...
private:
  struct PolynomialSheaf {
    int8_t candidateFields[MAX_STORAGE][4];  // Statically allocated candidate polynomials
    int candidateCount;                     // Count of candidate polynomials
    float scale;                            // Quantization scale

    PolynomialSheaf(float quantScale) : candidateCount(0), scale(quantScale) {}

    void addCandidateField(const int8_t* field) {
      if (candidateCount < MAX_STORAGE) {
        for (int i = 0; i < 4; i++) {
          candidateFields[candidateCount][i] = field[i];
        }
        candidateCount++;
      }
    }
  ...
  void constructPolynomialField(DataPoint* data, int start, int end, int8_t* field) {
    ...
    PolynomialSheaf sheaf(QUANTIZATION_SCALE);
    field[0] = sheaf.quantizeCoefficient(a3);
    field[1] = sheaf.quantizeCoefficient(a2);
    field[2] = sheaf.quantizeCoefficient(a1);
    field[3] = sheaf.quantizeCoefficient(a0);
  }
  ...
  void optimizeCoefficients(PolynomialSheaf& sheaf, DataPoint* data, int count, 
    ...
    // Apply constrained Monte Carlo refinement
