...
struct MeasurementMonitor {
    ...
    unsigned long times[4];   // Corresponding timestamps (ms)
...
void calculateDerivatives(MeasurementMonitor &monitor) {
    ...
    for (int i = 0; i < monitor.count - 1; i++) {
        unsigned long timeDiff = monitor.times[i] - monitor.times[i + 1];
...
float weightedPrediction(const MeasurementMonitor &monitor) {
    ...
    if (monitor.count >= 3) {
        ...
        float secondDerivative = deltaDerivative / deltaTime;
