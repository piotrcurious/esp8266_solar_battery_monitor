...
// Complete Kalman filter function
...
float SensorFilter::updateSensorReading(float newReading) {
    ...
    if (validCount == 0) {
        ...
        estimate = kalmanFilter(fallbackAverage);  // Use Kalman filter to handle fallback average
        ...
        estimate = kalmanFilter(filteredSum / validCount);
