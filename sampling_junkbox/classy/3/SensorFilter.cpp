...
SensorFilter::SensorFilter(int size, float outlierThreshold, float baseKalmanGain, float processNoise, float measurementNoise) {
    ...
    this->baseKalmanGain = constrain(baseKalmanGain, 0.001, 1.0); // Constrain base Kalman gain to prevent instability
    this->processNoise = max(processNoise, 0.0001); // Ensure positive process noise
    this->measurementNoise = max(measurementNoise, 0.0001); // Ensure positive measurement noise
...
float SensorFilter::kalmanFilter(float measurement, float gain) {
    ...
    gain = constrain(gain, 0.0001, 1.0);
...
float SensorFilter::updateSensorReading(float newReading) {
    ...
    if (validCount == 0) {
        ...
        estimate = kalmanFilter(fallbackAverage, max(kalmanGain * 0.1, 0.001));  // Use low Kalman gain but ensure it's not zero
