...
//      }      
  ...
  graphScheduler_init(); // mark all graphs in scheduler as complete so they are not redrawn with bogus values
...
//      Serial0.println(i+1);
    ...
    // Add the data to the rolling buffer
...
//graph scheduler subsystem to schedule graphs to be drawn . 
// TODO: handling pointers to graph data is missing, currently it just plots from hardcoded arrays so it is not really flexible. 
// see the junkbox foders for better solutions how to handle array decay. 

graphSchedule_struct graphSchedule[MAX_DISPLAYED_GRAPHS]  ; // define scheduler array

void scheduleGraphMulti(uint16_t dataSize, uint16_t graphPosX, uint16_t graphPosY,
                  uint16_t graphWidth, uint16_t graphHeight, uint8_t graph_index, uint16_t numLinesToDraw) {
                  graphSchedule[graph_index].dataSize       = dataSize;
                  graphSchedule[graph_index].graphPosX      = graphPosX;
                  graphSchedule[graph_index].graphPosY      = graphPosY;
                  graphSchedule[graph_index].graphWidth     = graphWidth;
                  graphSchedule[graph_index].graphHeight    = graphHeight;
                  graphSchedule[graph_index].lfsr           = 1;                // init lfsr seed
                  graphSchedule[graph_index].graphComplete  = false;            // mark as incomplete
                  graphSchedule[graph_index].graph_multi    = true;             // mark as multigraph to call multigraph logic
...
void scheduleGraphSingle(uint8_t data_pointer, uint16_t dataSize, uint16_t graphPosX, uint16_t graphPosY,
                  ...
                  graphSchedule[graph_index].graphWidth     = graphWidth;
                  graphSchedule[graph_index].graphHeight    = graphHeight;
                  graphSchedule[graph_index].graph_min      = graph_min;
                  graphSchedule[graph_index].graph_max      = graph_max;
                  graphSchedule[graph_index].graphColor     = graphColor;
                  graphSchedule[graph_index].clear_under    = clear_under;
...
uint16_t multi_galoisLFSR(bool *wrapped, uint8_t graph_index) {
    static uint16_t initialSeed = 1;  // Track the initial seed to detect wraparound
    uint16_t lsb = graphSchedule[graph_index].lfsr & 1;  // Get the least significant bit
    graphSchedule[graph_index].lfsr >>= 1;               // Shift the LFSR right by 1
    if (lsb) {
        // Apply the polynomial (0xB400 corresponds to the polynomial x^12 + x^11 + x^10 + x^4 + 1)
//        lfsr ^= 0xB400 ;  // Use the full 16bit polynomial
//        lfsr ^= 0xD008 ;  // Use the 12bit polynomial
...
void drawLinesUsingLFSR(uint8_t graph_index) {
    ...
    while (linesDrawn < graphSchedule[graph_index].numLinesToDraw) {
        uint16_t lineIndex = multi_galoisLFSR(&wrapped, graph_index); 
        // Ensure the generated index is within the range of valid buffered lines
//        if (lineIndex < lineBufferIndex) {
//            drawBufferedLineClr(lineIndex-1);  // the range starts from and is never 0 1 so shift by 1.
      if (graphSchedule[graph_index].graph_multi) {
    for (uint8_t i = 0 ; i < NUMBER_OF_GRAPHS; i++ ) {
          bool clear_under; 
          if (i == 0) {clear_under=true;} else {clear_under=false;}
          if (graph_order[i] >=0 ){
            plotGraphSection(minutes_buffer[graph_order[i]], lineIndex-1, graphSchedule[graph_index].dataSize, graphSchedule[graph_index].graphPosX, graphSchedule[graph_index].graphPosY,
                graphSchedule[graph_index].graphWidth, graphSchedule[graph_index].graphHeight, minutes_buffer_min[graph_order[i]], minutes_buffer_max[graph_order[i]], GRAPH_COLOR[graph_order[i]],clear_under);
          //plotGraph(minutes_buffer[graph_order[i]],minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X,displayHeight-(GRAPH_Y),
          //      minutes_buffer_min[graph_order[i]], minutes_buffer_max[graph_order[i]],GRAPH_COLOR[graph_order[i]], clear_under );       
          }  
        }
      } else { // if (graphSchedule[graph_index].graph_multi) {
        // single graph
      plotGraphSection(minutes_buffer[graphSchedule[graph_index].data_pointer], lineIndex-1, graphSchedule[graph_index].dataSize, graphSchedule[graph_index].graphPosX, graphSchedule[graph_index].graphPosY,
                graphSchedule[graph_index].graphWidth, graphSchedule[graph_index].graphHeight, graphSchedule[graph_index].graph_min, graphSchedule[graph_index].graph_max, graphSchedule[graph_index].graphColor,graphSchedule[graph_index].clear_under);
//      Serial0.println(lineIndex); // debug

      } // TODO : fix the storage and passing od pointer to any data array
            linesDrawn++;
//      Serial0.println(lineIndex); // debug
//        }
        // Break if the LFSR has wrapped around to avoid infinite looping
        if (wrapped) {
        graphSchedule[graph_index].graphComplete = true;
            break;
        }
        // break if more lines drawn than MAX_LINES possible
//        if (linesDrawn>MAX_LINES) {
...
void graphScheduler(){ // simple helper to update all the visible graphs.
                                          // in real application it is possible to choose which graphs update faster by increasing numLinesToDraw 
                                          // and calling individual update function individually.
                                          // this can be even updated dynamically depending on what is currently on the screen.
                                          // here we simply iterate to draw all the graphs.  
                                          // if they are not complete
  for (uint8_t i = 0 ; i< MAX_DISPLAYED_GRAPHS; i++) {
    graphComplete = true ; // all graph will be complete when all subelements will be not called
    if (!graphSchedule[i].graphComplete){
      graphComplete = false; // one of elements of the schedule was not complete , thus total graph is not complete... TODO: it's legacy of old logic, refactor to remove it. 
      drawLinesUsingLFSR(i);
    }     
  }
}
void graphScheduler_init(){ // simple helper to init graph scheduler, marking all graphs complete. for setup and reinit.
   for (uint8_t i = 0 ; i< MAX_DISPLAYED_GRAPHS; i++) {
    graphSchedule[i].graphComplete = true;          
  }
}
...
//                  minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true);     
      scheduleGraphSingle(current_graph, minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*5,displayHeight-(GRAPH_Y),
              minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true,0,64); // true=clear under, 0 - graph scheduler slot index, 64 - numLinesToDraw per blocking scheduler call. 
...
//      plotGraphMulti(minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*4,displayHeight-(GRAPH_Y)); 
      ...
      scheduleGraphMulti(minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*5,displayHeight-(GRAPH_Y),
              0,64); //  0 - graph scheduler slot index, 16 - numLinesToDraw per blocking scheduler call. 
...
//    if (!graphComplete) {drawRandomLines(8); }     // draw random lines, ensuring random number drawn hit the area not drawn before (high cpu but low bandwidth)
   ...
   // if (!graphComplete) {drawLinesUsingLFSR(CHUNKS_PER_DRAW); }  // draw basing on LFSR generated looping sequence. 
    ...
    graphScheduler(); // call the graph scheduler to display scheduled graphs 
    checkAndHandleEvents();

#ifdef GRAPH_TEST    
//    delay(10); // Update delay (for testing purposes, remove it )
...
//    BlueDisplay1.setFlagsAndSize(BD_FLAG_FIRST_RESET_ALL | BD_FLAG_USE_MAX_SIZE | BD_FLAG_TOUCH_BASIC_DISABLE, displayWidth,displayHeight);
    ...
    if (current_graph>=0) { // positive graph values mean real graphs
//blocking plot      plotGraph(minutes_buffer[current_graph], minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*4,displayHeight-(GRAPH_Y),
//              minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true);

      //template:  plotGraph(minutes_buffer, minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X, displayHeight-(GRAPH_Y), minutes_buffer_min, minutes_buffer_max,GRAPH1_COLOR,clear_under);
      
//template: void scheduleGraphSingle(uint8_t data_pointer, uint16_t dataSize, uint16_t graphPosX, uint16_t graphPosY,
//                  uint16_t graphWidth, uint16_t graphHeight, float graph_min, float graph_max, color16_t graphColor, bool clear_under , uint8_t graph_index, uint16_t numLinesToDraw) {
// todo: due to array decay when passing pointer, i no longer pass pointer, but i have graph data array hardcoded in graphing logic and i just pass the graph index. 
// this should be fixed so any graph data can be plotted. 
// see the junkbox for some solutions, i think the 'header struct' for array containing array parameters is good approach

// also we hardcode layout here, while layout should be defined by a widget layout struct, which is separate TODO and there is separate junkbox (new widget manager repository)

      scheduleGraphSingle(current_graph, minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*5,displayHeight-(GRAPH_Y),
              minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true,0,16); // true=clear under, 0 - graph scheduler slot index, 16 - numLinesToDraw per blocking scheduler call. 
...
//      plotGraphMulti(minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*4,displayHeight-(GRAPH_Y)); 
      ...
      scheduleGraphMulti(minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*5,displayHeight-(GRAPH_Y),
              0,16); //  0 - graph scheduler slot index, 16 - numLinesToDraw per blocking scheduler call. 
    }
...
//              minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true);
      scheduleGraphSingle(current_graph, minutes_dataArraySize, GRAPH_X, GRAPH_Y, displayWidth-GRAPH_X-LEGEND_LABEL_FONT_WIDTH*5,displayHeight-(GRAPH_Y),
              minutes_buffer_min[current_graph], minutes_buffer_max[current_graph],GRAPH_COLOR[current_graph],true,0,64); // true=clear under, 0 - graph scheduler slot index, 16 - numLinesToDraw per blocking scheduler call. 
    } else {
/*
      for (uint8_t i = 0 ; i < NUMBER_OF_GRAPHS; i++ ) {
        bool clear_under; 
...
//template: void scheduleGraphMulti(uint16_t dataSize, uint16_t graphPosX, uint16_t graphPosY,
//                  uint16_t graphWidth, uint16_t graphHeight, uint8_t graph_index, uint16_t numLinesToDraw) {
...
// vertical swipe adds or removes graphs to the graph order array
    ...
    printArray(LEGEND_LABEL_FONT_WIDTH*16,LEGEND_LABEL_FONT_SIZE); // print defined graph order array offset by debug window TODO:de-hardcode position
}
