...
void SensorFilter::setState(float estimate, float errorCovariance) {
    this->estimate = estimate;
    this->errorCovariance = errorCovariance;
    // Fill the readings buffer with the restored estimate to avoid skewing mean/stddev with zeros after wake
    for (int i = 0; i < arraySize; i++) {
        readings[i] = estimate;
    }
}

void SensorFilter::getState(float &estimate, float &errorCovariance) {
