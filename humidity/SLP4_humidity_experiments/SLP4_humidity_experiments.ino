//TODO : add APSTA mode to relay packets from other sensors . 
// problem : esp8266 stock wifiudp supports UDP recieve only on STA interface
// solution : port to ESPAsyncUDP :)

// bugs : you need to use nonos-sdk 2.2.x , otherwise re-association takes 3 seconds instead of 150ms.
// no idea why but sometimes it works with 3.0.5 , sometimes not.
...
#define SLEEP_DURATION 5 * 1000; // duration of sleep . 
#define SLEEP_PERIODS_TO_SEND_PACKET 3 // send packet only once per [x] sleep periods. other wakeups only update the filter.
...
#define ITERATIONS_PER_WAKEUP_SHT4x 8 // amount of sensor reading iterations per wakeup. 

#define ITERATIONS_OF_FILTER_PRELOAD 1024 // amount of iteraions to read sensors after reset (to stabilise filter)
...
void fpm_wakup_cb_func(void) {
  ...
  Serial.println(sleep_periods_elapsed);
  Serial.flush();  
  wifi_fpm_close();
}
...
telemetry_frame tframe; // for entire loop() and setup()
...
void preload_filters(){

   for (uint16_t i = 0 ; i < ITERATIONS_OF_FILTER_PRELOAD; i++) { 
  ...
  Serial.println("RSSI ok...");


}

void setup() {
  Serial.begin(115200); // no .
  pinMode(LED_PIN, INPUT);
...
//       Serial.println("Low heat for 0.1 second");
  ...
  yield_delay(2000); // wait until sensor cools down
  Serial.println("filters preload...");
  preload_filters();  
...
//  WiFi.config(local_ip, gateway, subnet, dns1, dns2)
  WiFi.config(_settings.ip_address, _settings.ip_gateway, _settings.ip_mask,_settings.ip_dns1,_settings.ip_dns2);
  WiFi.begin(ssid, password, _settings.wifi_channel, _settings.wifi_bssid, true);
  yield_delay(5); // small delay to get stuff propagated to OS, not sure if it is needed
...
if (transmit_phase) { Serial.print("sent in : "); Serial.println(radio_active_time); }
  ...
  yield_delay(20); // wait for the pin to settle
  if (digitalRead(MODE_PIN) == HIGH && !CONTINOUS_MODE ) {
    WiFi.persistent(false);
    WiFi.mode(WIFI_OFF);
    transmit_phase = false ; // indicate that we are offline
   // WiFi.forceSleepBegin(); // this should commit disabling the wifi but it breaks entering sleep instead. 
    yield_delay(20);
    Serial.flush();
    ...
    Serial.println("continous mode debug");             
