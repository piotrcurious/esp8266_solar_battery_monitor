...
SensorFilter::SensorFilter(int size, float outlierThreshold, float baseKalmanGain, float processNoise, float measurementNoise) {
    ...
    this->baseKalmanGain = constrainFloat(baseKalmanGain, 0.000001, 1.0); // Constrain base Kalman gain to prevent instability
    this->processNoise = (processNoise > 0.000001) ? processNoise : 0.000001; // Ensure positive process noise
    this->measurementNoise = (measurementNoise > 0.000001) ? measurementNoise : 0.000001; // Ensure positive measurement noise
