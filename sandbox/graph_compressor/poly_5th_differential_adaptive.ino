...
void solve(const std::vector<std::vector<double>>& A, const std::vector<double>& b, std::vector<double>& x) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (std::abs(mat[i][i]) > 1e-15) for (int j = i+1; j < n; j++) {
            double f = mat[j][i] / mat[i][i]; vec[j] -= f * vec[i];
            for (int k = i; k < n; k++) mat[j][k] -= f * mat[i][k];
        }
    }
    x.assign(n, 0); for (int i = n-1; i >= 0; i--) if (std::abs(mat[i][i]) > 1e-15) {
        double s = 0; for (int j = i+1; j < n; j++) s += mat[i][j] * x[j]; x[i] = (vec[i]-s)/mat[i][i];
    }
}

void fit(DP* d, int n, double* c) {
    if (n < 6) { memset(c, 0, 8*6); double s = 0; for(int i=0; i<n; i++) s += d[i].v; c[0] = s/n; return; }
    std::vector<std::vector<double>> A(6, std::vector<double>(6, 0.0)); std::vector<double> B(6, 0.0);
    double t0 = d[0].t; for (int i = 0; i < n; i++) {
...
void setup() {
    ...
    while (Serial.available()) {
        ...
        add(line.substring(0, ci).toFloat(), line.substring(ci+1).toFloat());
    }
    ...
    for(int i=0; i<sc; i++) {
        ...
        for(float t=p.tStart; t<=p.tEnd+0.001; t+=0.1) {
            float dt = t - p.tStart;
            double v=0, tp=1; for(int j=0; j<6; j++) { v+=cc[j]*tp; tp*=dt; }
            Serial.print("RESULT:"); Serial.print(t); Serial.print(":"); Serial.println((float)v);
        }
    }
}
void loop() {}
