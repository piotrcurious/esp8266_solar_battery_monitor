...
// Wrapper class to manage all variable clients without using std::vector
class VariableClientManager {
private:
    static const int maxClients = 10;  // Maximum number of clients
    VariableClient* clients[maxClients];
    int clientCount;

public:
    VariableClientManager() : clientCount(0) {}

    void addClient(VariableClient* client) {
        if (clientCount < maxClients) {
            clients[clientCount++] = client;
        } else {
            Serial.println("Error: Maximum number of clients reached.");
        }
    }
    ...
    // Method to retrieve a single variable or array by path
