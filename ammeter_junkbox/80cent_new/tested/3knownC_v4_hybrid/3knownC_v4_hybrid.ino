...
// Unified and Optimized 3knownC Logic v4.4 (Advanced Hybrid)
// Features:
...
bool fit_rc_profile() {
    ...
    // Improved initialization: Voc is likely higher than the last sample
    // if we haven't reached steady state.
    float last_v = voltage_samples[NUM_SAMPLES - 1];
    float first_v = voltage_samples[0];
    float f_voc = last_v + (last_v - first_v) * 0.2f;
    ...
    if (f_voc > 0.5) {
        open_circuit_voltage = 0.5f * f_voc + 0.5f * open_circuit_voltage;
        fitted_tau = 1.0f / f_lambda;
        resistance_tau_est = fitted_tau / KNOWN_CAPACITANCE;
        internal_resistance_src = 0.5f * resistance_tau_est + 0.5f * internal_resistance_src;
...
void controller_loop() {
    ...
    if (!is_calibrating) {
        ...
        // v4.4: Active Dither for Binning Exploration
        // Periodically perturb the load to hit the high/low bins
        static float dither_phase = 0;
        dither_phase += 0.2f;
        float dither = 0.03f * sinf(dither_phase);

        float err = target_v - s_volt;
        load = load - LEARNING_RATE_LOAD * err;
        load = constrain(load, 0, 1);

        float active_load = constrain(load + dither, 0, 1);
