...
#define VERIFICATION_PWM_PERTURBATION 0.1f // +/- 10% perturbation of current load  - not used in impulse method
#define VERIFICATION_SAMPLES 20 // Number of samples for verification - not used in impulse method
#define VERIFICATION_SAMPLE_INTERVAL 10 // Sample interval for verification - not used in impulse method
#define VOC_VERIFICATION_THRESHOLD 0.05 // 5% threshold for VOC verification (as a fraction)

// Impulse Verification Parameters
#define IMPULSE_VERIFICATION_INTERVAL 60000 // Interval for impulse VOC verification
#define IMPULSE_DURATION 5 // Duration of the PWM impulse in milliseconds
#define IMPULSE_PWM_DUTY 0.0f // Duty cycle during impulse (0 = off)
#define IMPULSE_RESPONSE_SAMPLES 15 // Number of samples to capture impulse response
#define IMPULSE_RESPONSE_SAMPLE_INTERVAL 2 // Interval between samples for impulse response
#define VOC_IMPULSE_THRESHOLD 0.05 // Threshold for Voc verification using impulse
#define TAU_IMPULSE_THRESHOLD 0.2  // Threshold for Tau verification using impulse (e.g., 20% deviation)
#define MIN_TAU_FOR_RATIO 1e-3   // Minimum tau value to use the tau ratio check, below this, tau check is skipped
...
unsigned long last_impulse_verification_time = 0; // Last time Impulse VOC verification was performed
...
float verification_voltage_samples[VERIFICATION_SAMPLES]; // Not used in impulse method, kept for compatibility
unsigned long verification_time_samples[VERIFICATION_SAMPLES]; // Not used in impulse method, kept for compatibility

float impulse_voltage_samples[IMPULSE_RESPONSE_SAMPLES];
unsigned long impulse_time_samples[IMPULSE_RESPONSE_SAMPLES];
...
bool verify_voc_impulse_response() {
    float current_pwm_value = pwm_value; // Store current PWM value

    // 1. Apply PWM Impulse and Sample voltage
    PWM_Instance->setPWM(PWM_PIN, frequency, current_pwm_value); // Ensure steady state before impulse
    delay(IMPULSE_DURATION * 2); // Wait for steady state

    PWM_Instance->setPWM(PWM_PIN, frequency, IMPULSE_PWM_DUTY); // Apply impulse (turn off PWM)
    for (int i = 0; i < IMPULSE_RESPONSE_SAMPLES; i++) {
        impulse_time_samples[i] = millis();
        delay(IMPULSE_RESPONSE_SAMPLE_INTERVAL);
        impulse_voltage_samples[i] = analogRead(SOURCE_PIN) * (SOURCE_VOLTAGE_RANGE / 1023.0);
    }
    PWM_Instance->setPWM(PWM_PIN, frequency, current_pwm_value); // Restore original PWM immediately


    // 2. Analyze Impulse Response (Advanced - VOC and Tau Verification)
    float measured_voc_peak = 0;
    for(int i = 0; i < IMPULSE_RESPONSE_SAMPLES; i++) { // Find actual peak voltage
        if (impulse_voltage_samples[i] > measured_voc_peak) {
            measured_voc_peak = impulse_voltage_samples[i];
        }
    }

    float tau_measured_rise_ratio = 0.0;
    float tau_expected_rise_ratio = 0.0;

    if (last_fitted_tau > MIN_TAU_FOR_RATIO && last_fitted_voc > 0 && IMPULSE_RESPONSE_SAMPLES > 1) {
        tau_measured_rise_ratio = (impulse_voltage_samples[1] - impulse_voltage_samples[0]) / measured_voc_peak; // Normalized rise in first interval
        float t_sample = (float)IMPULSE_RESPONSE_SAMPLE_INTERVAL / 1000.0;
        tau_expected_rise_ratio = (last_fitted_voc - (last_fitted_voc - last_fitted_b) * exp(-t_sample / (last_fitted_tau + 0.00001))) / last_fitted_voc; // Expected normalized rise based on fitted tau/Voc/B
    } else {
        tau_measured_rise_ratio = 0.0; // Skip tau check if last_fitted_tau is too small or invalid
        tau_expected_rise_ratio = 0.0;
    }


    // 3. Compare with expected values and thresholds (VOC and Tau)
    float voc_diff_percentage = fabs(measured_voc_peak - open_circuit_voltage) / open_circuit_voltage;
    float tau_diff_percentage = fabs(tau_measured_rise_ratio - tau_expected_rise_ratio);


    bool voc_verified = voc_diff_percentage <= VOC_IMPULSE_THRESHOLD;
    bool tau_verified = true; // Default to true, only check if tau is valid for ratio calculation
    if (last_fitted_tau > MIN_TAU_FOR_RATIO) { // Only perform tau check if tau is valid
        tau_verified = tau_diff_percentage <= TAU_IMPULSE_THRESHOLD;
    }


    if (voc_verified && tau_verified) {
        #ifdef FITTING_DEBUG
        Serial.print(F("Impulse VOC Verification: PASS, Peak Voc: ")); Serial.print(measured_voc_peak);
        Serial.print(F(", Voc Diff%: ")); Serial.print(voc_diff_percentage*100.0);
        if (last_fitted_tau > MIN_TAU_FOR_RATIO) {
            Serial.print(F(", Tau Diff Ratio: ")); Serial.println(tau_diff_percentage);
        } else {
            Serial.println(F(", Tau check skipped (Tau too small)"));
        }

        #endif
        return true; // VOC and Tau verified successfully with impulse
    } else {
        #ifdef FITTING_DEBUG
        Serial.print(F("Impulse VOC Verification: FAIL, Peak Voc: ")); Serial.print(measured_voc_peak);
        Serial.print(F(", Voc Diff%: ")); Serial.print(voc_diff_percentage*100.0);
         if (last_fitted_tau > MIN_TAU_FOR_RATIO) {
            Serial.print(F(", Tau Diff Ratio: ")); Serial.println(tau_diff_percentage);
        } else {
        ...
        #endif
        return false; // VOC or Tau verification failed with impulse
    }
}
...
last_impulse_verification_time = millis(); // Initialize impulse verification time
...
// Check if it is time to verify VOC using Impulse Response
if (millis() - last_impulse_verification_time > IMPULSE_VERIFICATION_INTERVAL) {
    verify_voc_impulse_response(); // Perform VOC verification using impulse
    last_impulse_verification_time = millis();
}


// Check if it is time to verify VOC using Oscillation (original method - less frequent)
if (millis() - last_voc_verification_time > VOC_VERIFICATION_INTERVAL * 2 ) { // Verify less frequently with oscillation
    verify_voc(); // Perform VOC verification using Oscillation
