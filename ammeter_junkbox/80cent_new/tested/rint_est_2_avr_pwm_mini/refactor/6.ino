...
struct Config {
    ...
    static constexpr float MIN_DUTY = 0.05;
...
private:
    ...
    // State variables
    float temperature = Config::TEMP_REFERENCE;
    float duty_cycle = 0;
    ...
    void updateTemperatureCompensation() {
        float temp_diff = temperature - Config::TEMP_REFERENCE;
        impedance.R_inductor = Config::R_L * (1 + Config::TC_R_L * temp_diff);
        impedance.R_fet = Config::R_DS_ON * (1 + Config::TC_R_DS * temp_diff);
    }
    
    float calculateSwitchingLosses() {
        float P_sw = 0;
        // Switching losses
        P_sw += 0.5 * source_voltage * inductor_current * Config::F_SW * 
                (Config::T_DEAD + Config::T_SW * (duty_cycle + (1 - duty_cycle)));
        // Conduction losses
    ...
    void updateImpedanceModel() {
        float omega = 2 * PI * Config::F_SW;
        
        // Update reactances
        impedance.X_L = omega * Config::L;
        impedance.X_C = 1 / (omega * Config::C_IN);
    ...
    bool checkProtection() {
        if (source_voltage > Config::V_MAX || 
            load_voltage > Config::V_MAX || 
            inductor_current > Config::I_MAX || 
    ...
    void updateControl() {
        ...
        float target_voltage = Config::VOLTAGE_FACTOR * source_voltage * 
                             (1 - switching_losses / (source_voltage * input_current));
        
        // Update error with impedance compensation
        float voltage_drop = impedance.Z_total * input_current;
        error = (error + (target_voltage - (load_voltage + voltage_drop))) / 2;
        
        // Update duty cycle with adaptive learning rate
        float adaptive_rate = Config::LEARNING_RATE * (1 - duty_cycle * duty_cycle);
        duty_cycle = duty_cycle - adaptive_rate * error;
        duty_cycle = constrain(duty_cycle, Config::MIN_DUTY, Config::MAX_DUTY);
    ...
    float readTemperature() {
        ...
        return (analogRead(Config::TEMP_PIN) * (5.0 / 1023.0) - 0.5) * 100;
    }
...
public:
    ...
    void update() {
        ...
        // Protection check
        if (now - last_protection_check >= Config::PROTECTION_CHECK_INTERVAL) {
            checkProtection();
