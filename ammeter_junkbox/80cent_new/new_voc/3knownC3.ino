...
    }
    ...
    float fitRCProfile() {
        ...
        for (int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
            ...
            // Gauss-Newton update - Solving for delta_Voc and delta_tau using a simplified 2x2 system solver
            float denominator = (sum_jacobian_Voc_sq * sum_jacobian_tau_sq - pow(sum_jacobian_Voc_tau, 2));
            float delta_Voc = (sum_residuals_Voc * sum_jacobian_tau_sq - sum_residuals_tau * sum_jacobian_Voc_tau) / denominator;
            float delta_tau = (sum_residuals_tau * sum_jacobian_Voc_sq - sum_residuals_Voc * sum_jacobian_Voc_tau) / denominator;


            Voc_estimate += delta_Voc;
            tau_estimate += delta_tau;
...
        Serial.print(F(",Fault:")); Serial.print(fault_condition);
        Serial.print(F(",rint:")); Serial.println(rint); // Debug output for rint
    }
};
