#include <Arduino.h>
#include <math.h>
#include <ArduinoEigen.h>  // Use ArduinoEigen library
...
void fitCurve(const CurveData *curve, int size, int degree, float *coefficients, float &fitError) {
  // Use ArduinoEigen for matrix-based least squares fitting
  // Create matrices with dynamic memory allocation
  MatrixXf A(size, degree + 1);
  VectorXf b(size);
