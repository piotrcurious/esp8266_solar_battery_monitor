...
void probeFullCurve(bool isDrop, CurveData *curve, int &size, float *coefficients);
void fitCurve(const CurveData *curve, int size, int degree, float *coefficients);
...
void measureNoLoadVoltage() {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    if (millis() - start > 100) break; // Adjust measurement delay based on time elapsed
  }
